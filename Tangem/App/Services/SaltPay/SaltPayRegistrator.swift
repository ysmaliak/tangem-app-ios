...
class SaltPayRegistrator {
    ...
    private var accessCode: String? = nil // "111111" //TODO: remove
...
//                    .eraseToAnyPublisher()
            ...
            .flatMap { [weak self] response -> AnyPublisher<RegisterWalletResponse, Error> in
                ...
                return self.api.registerWallet(request: request)
                    // .replaceError(with: RegisterWalletResponse(error: nil, errorCode: nil, success: true)) //TODO: remove
                    // .setFailureType(to: Error.self) //TODO: remove
                    .eraseToAnyPublisher()
            }
            .sink { [weak self] completion in
                if case let .failure(error) = completion {
...
//        }
        ...
        // self.state = .kycStart //TODO: remove test
    }
...
fileprivate class RegistrationTask: CardSessionRunnable {
    ...
    private func generateOTP(in session: CardSession, completion: @escaping CompletionResult<RegistrationTaskResponse>) {
        ...
        session.viewDelegate.showAlertMessage("Card is preparing...") // TODO: localize or remove
    ...
    private func attestWallet(in session: CardSession, completion: @escaping CompletionResult<RegistrationTaskResponse>) {
        ...
        session.viewDelegate.showAlertMessage("Binding the wallet...") // TODO: localize or remove
    ...
    private func prepareTransactions(in session: CardSession, completion: @escaping CompletionResult<RegistrationTaskResponse>) {
        ...
        session.viewDelegate.showAlertMessage("Preparing blockchain transactions...") // TODO: localize or remove
    ...
    private func signTransactions(_ transactions: [CompilledEthereumTransaction],
        ...
        session.viewDelegate.showAlertMessage("Signing blockchain transactions...") // TODO: localize or remove
        ...
        cmd.run(in: session) { result in
            ...
            case .success(let response):
                ...
                session.viewDelegate.showAlertMessage("Card interaction complete successfully") // TODO: localize or remove
