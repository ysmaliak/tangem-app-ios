    ...
    private func restore() {
        updateQueue.async { [weak self] in
            ...
            if let oldSessionsObject = UserDefaults.standard.object(forKey: self.sessionsKey) as? Data {
                ...
                // TODO: Refactor. We can use seed keys as identifiers for sessions now.
...
extension WalletConnectService: ServerDelegate {
    ...
    private func askForConnect(walletInfo: WalletInfo, dAppInfo: Session.DAppInfo, server: Server, completion: @escaping (Session.WalletInfo) -> Void) {
        ...
        let onSelectChain: (BlockchainNetwork) -> Void = { [cardModel] selectedNetwork in
            let wallet = cardModel.walletModels! // TODO: remove force unwrap with state cleaning
        ...
        let onSelectChainRequested = { [cardModel] in
            let availableChains = cardModel.walletModels! // TODO: remove force unwrap with state cleaning
