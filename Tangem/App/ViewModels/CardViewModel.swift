...
class CardViewModel: Identifiable, ObservableObject {
    ...
    private var _userWallet: UserWallet! // TODO: remove once we get rid of CardInfo
    ...
    private func bind() {
        signer.signPublisher.sink { [unowned self] card in
            self.cardInfo.card = CardDTO(card: card) // TODO: We need to update only changed parts of card
            self.config = UserWalletConfigFactory(cardInfo).makeConfig()
