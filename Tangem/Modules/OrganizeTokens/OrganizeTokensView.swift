...
struct OrganizeTokensView: View {
    ...
    init(viewModel: OrganizeTokensViewModel) {
        Self.setupAppearanceIfNeeded()  // FIXME: Andrey Fedorov - Is it better to setup on appear?
    ...
    private var tokenListHeader: some View {
        OrganizeTokensHeaderView(viewModel: viewModel.headerViewModel)
            ...
            .padding(.horizontal, Constants.contentHorizontalInset)
            .background(
                VisualEffectView(style: .systemUltraThinMaterial)   // TODO: Andrey Fedorov - Visibility should depend on the content offset
    ...
    private static func setupAppearanceIfNeeded() {
        // TODO: Andrey Fedorov - setup conditionally only for this screen using custom containers!
