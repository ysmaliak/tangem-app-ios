...
final class OrganizeTokensViewModel: ObservableObject, Identifiable {
        ...
        onSave
            ...
            .flatMapLatest { viewModel, _ in
                let walletModelIds = viewModel
                    .sections
                    .flatMap(\.items)
                    .map { $0.id as! ListItemViewModelIdentifier } // TODO: Andrey Fedorov - Get rid of force unwrap here
                    .map(\.walletModelId)
    ...
    private static func map(
        ...
        case .withoutDerivation(let userToken, let blockchainNetwork, let walletModelID):
            // TODO: Andrey Fedorov - Improve item VM for cases when there is no derivation
            let identifier = ListItemViewModelIdentifier(
